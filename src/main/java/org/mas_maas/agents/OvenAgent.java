...
public class OvenAgent extends BaseAgent {
    ...
    protected void setup() {
        ...
        this.register(this.bakingManagerName, "JADE-bakery");
        ...
        public void action() {
            ...
            if (msg != null) {
                ...
                if (!oven.isAvailable()){
                    ...
                    reply.setContent("Oven is taken");
                }
                else{
                    oven.setAvailable(false);
                    String content = msg.getContent();
                    System.out.println("***** > " + getAID().getLocalName() + " WILL perform Baking for "
                        + msg.getSender() + " baking information -> " + content);

                    BakingRequest bakingRequest = JSONConverter.parseBakingRequest(content);

                    reply.setPerformative(ACLMessage.INFORM);
                    reply.setContent("Baking request was received " + content);
